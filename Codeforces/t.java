...
public class t {
    ...
    static void functi1(Scanner scanner) {
        ...
        for (int i = 0; i < t; i++) {
            k[i] = scanner.nextInt(); // Read k values
        }
 
        int maxk = 0;
        for (int i = 0; i < t; i++) {
            maxk = Math.max(maxk, k[i]); // Find the maximum k value
        }
 
        long[] pow2 = new long[maxk + 1];
        pow2[0] = 1; // 2^0 = 1
        for (int i = 1; i <= maxk; i++) {
            pow2[i] = (pow2[i - 1] * 2) % MOD; // Calculate powers of 2 modulo MOD
        }
 
        for (int i = 0; i < t; i++) {
            System.out.println(pow2[k[i]]); // Output the result for each k
        }
        }
